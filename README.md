...
`scripts/21_nominal_excess_and_concordance.R` re-derives the nominal-finding counts, the binomial tests against chance, the cross-endpoint concordance statistics and the effect of the FDR correction denominator, all from the result tables above.
...
- `S19_coverage_summary.csv` and `S19_diagnostic_coverage.csv`
...
`results/figures/` also contains files under the numbering used during analysis: `Fig1_flowchart.*`, `Fig2_forest.*` and `Fig3_leaveoneout.*`. These predate the final figure order. `Fig2_forest` is the manuscript's Supplementary Figure S1 and `Fig3_leaveoneout` is Supplementary Figure S2; the manuscript's Figure 2 is the volcano plot and Figure 3 is the threshold-instability plot. The files named `Figure_*` follow the manuscript numbering and are the ones submitted.

The supplementary workbook generated by `make_supplementary.py` writes `supplementary/Supplementary_Tables.xlsx` containing Supplementary Tables S1-S20. Every sheet is built from a file in `results/tables/`, so the workbook can be rebuilt from a clean checkout: S1-S12 from the MR result tables, S13 from `S13_chen_reanalysis.csv`, S14-S16 from the post-hoc rare-variant outputs, S17 from the endpoint definitions, S18 from `S18_phenotype_power.csv`, and S19-S20 from the diagnostic-coverage outputs. Earlier versions carried S13-S16 forward from an existing copy of the workbook, which meant those sheets could not be regenerated from scratch. S18 is read from `S18_phenotype_power.csv` rather than recomputed, because instrument counts and R2 must come from the harmonised instrument set used by `12b_immune_mr.R` in order to match Supplementary Tables S1 and S2.
