...
# exposure (continuous, standardised): r2 = 2*f*(1-f)*b^2.
# The outcome is binary and its coefficients are log odds ratios; applying the same formula to
# them does NOT put the two sides on a common scale, and it understates the outcome-side variance
# rather than overstating it. The correct treatment converts the log odds ratio to the liability
# scale using the population prevalence (Hemani 2015). Because that conversion is not implemented
# here, the test is biased towards declaring the exposure-to-outcome orientation, and the
# manuscript reports it as a formal check only and draws no directional conclusion from it.
